...
public class LogFragment extends Fragment {
    ...
    Runnable updaterRunner = new Runnable() {
        public void run() {
            ...
            if (listDataBuffer == null || listData == null || listDataUnfiltered == null) {
                MyLog.d(4, "LogFragmentUpdater data is null, exiting");
                return;
            }
                ...
                synchronized (listData) {
                    synchronized (listDataUnfiltered) {
                        for (ListItem item : listDataBuffer) {
                            ...
                            item.print(getActivity().getApplicationContext());
                            i++;
                        }
    ...
    protected class ListItem {
        ...
        boolean empty() {
            return !(in != null || out != null || proto != null || srcAddr != null || srcPort > 0 || dstAddr != null || dstPort > 0 || len > 0 | timestamp != 0);
        }
        ...
        public void print(Context context) {
            ...
            if (srcAddr != null)
                Log.d("Tomer: source address", srcAddr);
            if (dstAddr != null) {
                Log.d("Tomer: dest address", dstAddr);
                if (hostNames == null)
                    hostNames = new HostNames(context);
                Log.d("Tomer: dest name", hostNames.getName(dstAddr));
            }
            if (len != 0)
                Log.d("Tomer: len", String.valueOf(len));
            if (srcPort != 0)
                Log.d("Tomer: src port", String.valueOf(srcPort));
            if (dstPort != 0)
                Log.d("Tomer: dst port", String.valueOf(dstPort));
            if (timestamp != 0)
